...
> 结论：OpenClaw 的记忆不是一个简单的 `MEMORY.md` 文件，也不是一个简单向量库。它更像“Markdown 记忆材料 + SQLite 本地索引 + 记忆工具 + 后台整理机制”的组合。
...
这意味着：`MEMORY.md` 是记忆材料之一；SQLite 是检索缓存和索引；`memory_search` 是 Agent 接触记忆系统的工具入口；真正的记忆生命周期横跨文件、索引、工具和后台任务。
...
这个同步通常不阻塞用户对话。它的作用是让当前工作区的记忆文件尽快进入索引。
...
所以用户或 Agent 写入 `MEMORY.md` 后，新的内容要等 watcher 和同步流程跑完，才稳定进入 SQLite 检索索引。
...
如果索引完全为空，会强制同步；如果索引只是过期，通常不会为了刷新阻塞本次搜索。
...
切片逻辑按行累积文本，达到预算后 flush 一个 chunk，然后保留尾部 overlap 作为下一个 chunk 的开头。它还对中文等 CJK 内容做了更保守的字符估算，避免一长段中文被错误估成很少 token。
...
注意：切片保存的是行号范围。后续 `memory_search` 返回结果时可以告诉 Agent 片段来自哪个文件、哪几行；`memory_get` 可以再按来源读取更完整原文。
...
同步时，OpenClaw 会拿当前文件 hash 和 `files.hash` 比较。如果没变，就跳过；如果变了，就重新切片并更新索引。
...
Tokenizer 默认可以是 `unicode61`。LobsterAI 当前更适合中文场景时，会倾向配置 `trigram`。`trigram` 对中文、项目代号、无空格术语更友好。
...
这里要注意：**写入 Markdown 文件不等于已经进入 SQLite 索引。**

写入只是改变了记忆材料。之后要等索引同步。
...
- SQLite 之前没有任何 chunk：本次搜索会强制同步，然后能搜到；
- SQLite 已经有旧索引：本次搜索可能先用旧索引返回，同时后台刷新。
...
这里要注意一个顺序：`PortableGit` 和 `变更单` 是被召回 chunk 里的内容，不应该提前出现在 query 里。Agent 可以在第一次搜索命中后，再基于结果继续追问或调用 `memory_get` 精读。
...
这个机制说明 OpenClaw 有“短期内容长期化”的思路。但它仍然是个人/工作区级别的自动整理，不是企业知识审批。
...
所以 SQLite 中旧文本不会作为同一文件的当前 chunk 长期保留。它是索引，不是版本历史。

如果企业需要版本历史，不能依赖 OpenClaw SQLite，需要在 LobsterAI 或企业后端单独设计版本表和审计表。

## 5. `memory_search` 到底怎么工作
...
- “发版注意事项”匹配“上线前检查”；
- “审批流程”匹配“变更单”；
- “Windows 打包风险”匹配“PortableGit 验证”。
...
Builtin 和 QMD 的根本区别是：**Builtin 的 SQLite 表由 OpenClaw 自己创建和写入；QMD 的 SQLite 索引由 QMD 创建和维护，OpenClaw 只负责配置 collection、触发 update/embed、调用搜索命令、把结果转成统一的 `memory_search` 返回格式。**
...
本节的 QMD 表结构以 [QMD 上游 README](https://github.com/tobi/qmd) 和 [`src/store.ts`](https://raw.githubusercontent.com/tobi/qmd/main/src/store.ts) 当前实现为口径；实际部署时仍应以目标机器 `index.sqlite` 的 `.schema` 为最终依据。
...
注意：下面是站在 QMD 自己的视角讲。QMD 的 schema 归 QMD 管，不归 OpenClaw 管。OpenClaw 只是设置环境变量、调用 QMD 命令，并在需要定位原文时读取 QMD 索引中的文档位置。
...
`active` 是软删除标记。文件从磁盘删除后，QMD 不一定立刻物理删除记录，而是把 `active` 置为 `0`，使它不参与搜索。
...
| 原文定位 | `chunks.path` | `documents.collection + documents.path` |
...
```text
qmd search "LobsterAI 发版前有什么特别要注意的吗？" --json -n 6 -c project-lobsterai
```

`searchMode = vsearch`：

```text
qmd vsearch "LobsterAI 发版前有什么特别要注意的吗？" --json -n 6 -c project-lobsterai
```

`searchMode = query`：

```text
qmd query "LobsterAI 发版前有什么特别要注意的吗？" --json -n 6 -c project-lobsterai
```
...
| 模式 | 查什么 | 类比 Builtin |
| --- | --- | --- |
| `search` | QMD 的词法/BM25 索引 | 类似查 `chunks_fts` |
| `vsearch` | QMD 的向量索引 | 类似查 `chunks_vec` |
| `query` | QMD 的混合查询，可能包含 lex、vec、hyde/rerank | 类似 hybrid search，但由 QMD 决定融合方式 |

站在 QMD 自己的视角，三个模式大致对应三条底层路径。
...
这和 builtin 最大的差异是：builtin 的 hybrid merge 在 OpenClaw 里做；QMD 的 hybrid、query expansion、rerank 在 QMD 内部做，OpenClaw 只拿最终 JSON 结果。
...
QMD 的搜索结果来自 QMD index，但 `memory_get` 精读时仍然回到 Markdown 文件。
...
| 生命周期阶段 | Builtin | QMD |
| --- | --- | --- |
| 记忆材料 | `MEMORY.md`、`memory/`、`extraPaths` | collection 指向的 Markdown |
| 入库触发 | `syncMemoryFiles()` | `qmd update` |
| 文件状态 | `files` 表 | QMD `documents` 表 |
| 原文存储 | `chunks.text` 保存片段 | `content.doc` 保存整篇原文 |
| 切片 | OpenClaw `chunkMarkdown()` | QMD `qmd embed` 时按约 900 tokens、15% overlap 切向量 chunk |
| 全文搜索 | OpenClaw 查 `chunks_fts` | `qmd search` |
| 向量搜索 | OpenClaw 查 `chunks_vec` | `qmd vsearch` |
| 混合搜索 | OpenClaw merge vector + keyword | `qmd query` |
| 结果结构 | `path/start_line/end_line/text/score` | `docid/collection/file/snippet/score` |
| 结果统一 | 原生就是 `MemorySearchResult` | OpenClaw 转成 `MemorySearchResult` |
| 精读 | 按 workspace path 读文件 | 按 collection root 安全读文件 |
...
但 QMD collection 仍然不是企业权限模型。它只能表达“索引哪些资料、按什么集合搜索”，不能表达：
...
## 7. active-memory：为什么 Agent 不一定要自己想起来搜索

只靠 prompt 告诉 Agent“需要时搜索记忆”是不稳定的。模型可能忘记搜索，也可能搜索太晚。

OpenClaw 的 active-memory 提供了一个主动路径：
...
所以 LobsterAI 当前更像 OpenClaw 记忆的产品化外壳，不是替代 OpenClaw 记忆机制。
...
这部分不能依赖 OpenClaw 的本地 SQLite 完成。SQLite 只知道当前被索引 chunk，不知道组织制度的审批状态和版本历史。

## 13. 是否需要改 OpenClaw 源码
...
这些能力可以在 LobsterAI 层实现，也可以部分贡献给 OpenClaw。但公司组织架构、权限、审批流不应该塞进 OpenClaw 核心。
...
如果只做个人记忆，改 LobsterAI 的 `MEMORY.md` 管理和 OpenClaw 配置就够了。

如果做项目记忆，可以先不改 OpenClaw，用后端结构化存储 + 客户端只读同步 + Markdown/QMD 物化 + `extraPaths` 验证。

如果做组织记忆，必须引入企业级 memory service，并把 ACL、citation、版本、审计和检索尽量放在服务端。QMD collection 只能作为低风险、短 TTL 的客户端缓存或项目级索引，不应成为组织记忆事实源。

一句话：OpenClaw SQLite 和 QMD index 里的内容都是客户端检索视图，不是企业记忆本身；项目记忆和组织记忆的真源必须在企业后端，由客户端受控同步或发起修改请求，再把允许的结果交给 OpenClaw 召回。
